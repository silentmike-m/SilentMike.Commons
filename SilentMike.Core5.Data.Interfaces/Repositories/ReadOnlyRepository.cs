...
using SilentMike.Core5.Data.Interfaces.Models;
using System;
...
namespace SilentMike.Core5.Data.Interfaces.Repositories
{
        ...
        where TUser : IdentityUser
    {
        ...
        public IEnumerable<T> GetAllOrdered<TKey>(IPageInfo pageInfo)
        {
            IQueryable<T> query = DbSet;
        ...
        public IEnumerable<T> GetAllOrdered<TKey>(List<Expression<Func<T, object>>> includes, IPageInfo pageInfo)
        {
            IQueryable<T> query = DbSet;
            query = GetIncluded(query, includes);
            query = GetPaged<TKey>(query, pageInfo);
        ...
        public IEnumerable<T> FindByOrdered<TKey>(Expression<Func<T, bool>> predicate, IPageInfo pageInfo)
        {
            var query = DbSet.Where(predicate);
            query = GetPaged<TKey>(query, pageInfo);
            return query.AsEnumerable();
        ...
        public IEnumerable<T> FindByOrdered<TKey>(Expression<Func<T, bool>> predicate,
            List<Expression<Func<T, object>>> includes, IPageInfo pageInfo)
        {
            var query = DbSet.Where(predicate);
            query = GetIncluded(query, includes);
            query = GetPaged<TKey>(query, pageInfo);
        ...
        public IEnumerable<T> FindByOrderedWithId<TKey>(Expression<Func<T, bool>> predicate, IPageInfo pageInfo, int id)
        {
            var query = DbSet.Where(predicate);
        ...
        private IQueryable<T> GetPaged<TKey>(IQueryable<T> query, IPageInfo pageInfo)
        {
            if (pageInfo == null)
                return query;
        ...
        private IEnumerable<T> GetPagedWithId<TKey>(IQueryable<T> query, IPageInfo pageInfo, int id)
        {
            if (pageInfo == null)
                return query;
            var prop = string.IsNullOrEmpty(pageInfo.OrderByPropertyName) ?
        ...
        private IOrderedQueryable<T> GetOrderedQuery<TKey>(IQueryable<T> query, IPageInfo pageInfo)
        {
